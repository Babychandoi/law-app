    ...
    setTimeout(async () => {
      try {
        const formData = {
          ...
          category:
            filters.category !== '-1'
              ? jobListingConfig.categories.find((category) => category.value === filters.category)
                  ?.label || ''
              : '',
          jobType:
            filters.jobType !== '-1'
              ? jobListingConfig.jobTypes.find((jobType) => jobType.value === filters.jobType)
                  ?.label || ''
              : '',
          location:
            filters.location !== '-1'
              ? jobListingConfig.locations.find((location) => location.value === filters.location)
                  ?.label || ''
              : '',
        };
    ...
    <div className="container mx-auto px-4 py-8" style={{ marginTop: '20px' }}>
      ...
      <div className="mb-12 text-center">
        ...
        <p className="text-xl text-gray-600 max-w-2xl mx-auto mb-8">
          Gia nhập đội ngũ chuyên nghiệp của Luật Poip Legal - Nơi bạn phát triển sự nghiệp cùng
          chúng tôi
        </p>
