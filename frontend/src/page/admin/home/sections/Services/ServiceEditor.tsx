...
const resolveImg = (v?: string) =>
  !v ? '' : v.startsWith('http') || v.startsWith('/') ? v : `https://minio.luatpoip.com/images/${v}`;
            ...
            <div key={i} className="rounded-lg border border-gray-200 bg-white p-4">
              ...
              <div className="grid gap-3 sm:grid-cols-[100px_1fr]">
                <input
                  ...
                  placeholder="01"
                  onChange={(e) =>
                    setProcess(process.map((s, x) => (x === i ? { ...s, step: e.target.value } : s)))
                  }
                />
                <input
                  className={inputCls}
              ...
              <textarea
                ...
                onChange={(e) =>
                  setProcess(
                    process.map((s, x) => (x === i ? { ...s, description: e.target.value } : s))
                  )
                }
              />
          ...
          {pricing.map((plan, i) => (
            <div key={i} className="rounded-lg border border-gray-200 bg-white p-4">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="flex gap-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setPricing(
                        pricing.map((p, x) => (x === i ? { ...p, currency: e.target.value } : p))
                      )
                    }
                  />
                </div>
              ...
              <textarea
                ...
                onChange={(e) =>
                  setPricing(
                    pricing.map((p, x) => (x === i ? { ...p, description: e.target.value } : p))
                  )
                }
              />
              <textarea
                ...
                placeholder={'Các quyền lợi, mỗi dòng một mục\nVD: Tra cứu miễn phí\nSoạn hồ sơ trọn gói'}
                onChange={(e) =>
                  setPricing(
                    pricing.map((p, x) =>
              ...
              <label className="mt-3 inline-flex items-center gap-2 text-sm text-gray-700">
                <input
                  ...
                  onChange={(e) =>
                    setPricing(
                      pricing.map((p, x) => (x === i ? { ...p, featured: e.target.checked } : p))
                    )
                  }
                />
