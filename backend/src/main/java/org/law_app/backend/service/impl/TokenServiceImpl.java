...
public class TokenServiceImpl implements TokenService {
  ...
  private String generateToken(User user, String type, long validitySeconds) {
    ...
    JWTClaimsSet claimsSet =
        new JWTClaimsSet.Builder()
            ...
            .issueTime(new Date())
            .expirationTime(
                new Date(Instant.now().plus(validitySeconds, ChronoUnit.SECONDS).toEpochMilli()))
  ...
  @Override
  public SignedJWT verifyToken(String token, boolean allowExpired)
  ...
  public void saveRefreshToken(String userId, String refreshToken, long ttlSeconds) {
    redisTemplate
        .opsForValue()
        .set("refresh:" + userId, refreshToken, ttlSeconds, TimeUnit.SECONDS);
  }
