...
const updateGuestOnlineStatus = async (_guestId: string, _isOnline: boolean) => {
...
const ChatBox: React.FC<ChatBoxProps> = ({ onError, maxMessages = 100, onClose }) => {
  ...
  const connectWebSocket = useCallback(() => {
    if (!WS_URL) {
      const errorMsg =
        'WebSocket URL không được cấu hình. Vui lòng kiểm tra REACT_APP_CHAT_WS_URL.';
      setApiError(errorMsg);
    ...
    try {
      ...
      const client = new Client({
        ...
        onStompError: (frame) => {
          setConnectionState(ConnectionState.ERROR);
          setApiError(
            'Lỗi kết nối WebSocket: ' + (frame.headers?.message || 'Không có chi tiết lỗi')
          );
          updateGuestOnlineStatus(guestId, false);
      ...
      <div className="bg-brand-goldDark text-white px-4 py-4 flex items-center justify-between shadow-sm relative overflow-hidden">
        ...
        <div className="flex items-center space-x-3 relative z-10">
          <div className="relative">
            ...
            <div
              className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-white ${connectionStatus.color}`}
            ></div>
          </div>
          <div>
