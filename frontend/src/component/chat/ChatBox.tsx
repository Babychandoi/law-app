...
const ChatBox: React.FC<ChatBoxProps> = ({ onError, maxMessages = 100, onClose }) => {
  ...
  return (
    <div 
      className="transition-all duration-300 ease-in-out w-96 max-h-[600px]"
      style={{ zIndex: 1000 }}
    >
      {/* Chat Header */}
      <div className="bg-gradient-to-r from-blue-500 to-blue-600 text-white rounded-t-xl px-4 py-3 flex items-center justify-between shadow-lg">
        <div className="flex items-center space-x-3">
          <div className={`w-3 h-3 rounded-full ${connectionStatus.color} ${isConnected ? 'animate-pulse' : ''}`}></div>
          <div>
      ...
      {/* Chat Body */}
      <div className="bg-white rounded-b-xl shadow-xl border-l border-r border-b border-gray-200 flex flex-col h-[544px]">
        {/* Messages Container */}
                      ...
                      <div className="flex items-center justify-end mt-1 space-x-1">
                        ...
                        {msg.status === 'failed' && (
                          <div className="flex items-center space-x-1">
                            ...
                            <span className="text-xs text-white/80">Thất bại</span>
                          </div>
                        )}
                      </div>
                    )}
