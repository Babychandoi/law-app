...
public class ServiceServiceImpl implements ServiceService {
  ...
  public List<ServiceResponse> getServices(String title) {
    ...
    Map<String, List<ChildrenServices>> childrenByParentId =
        childrenServiceRepository.findByParentServiceIn(services).stream()
            .collect(Collectors.groupingBy(child -> child.getParentService().getId()));

    List<ServiceResponse> serviceResponses = new ArrayList<>();
    for (Services service : services) {
      List<ServiceResponse> childResponses =
          childrenByParentId.getOrDefault(service.getId(), List.of()).stream()
  ...
  public List<ChildrenServiceResponse> getChildrenServiceById(String id) {
    try {
      ...
      List<ChildrenServices> childrenService =
          childrenServiceRepository.findByParentService(services);
      return childrenService.stream().map(serviceMapper::toChildrenServiceResponse).toList();
    } catch (Exception e) {
  ...
  public ToToResponse getToTo() {
    ...
    return ToToResponse.builder()
        ...
        .importants(
            company.getImportants().stream().map(serviceMapper::toImportantResponse).toList())
        .locations(company.getLocations().stream().map(serviceMapper::toLocationResponse).toList())
        .phoneContacts(
            company.getPhoneContacts().stream().map(serviceMapper::toPhoneContactResponse).toList())
        .socials(company.getSocials().stream().map(serviceMapper::toSocialResponse).toList())
        .build();
  }
