...
1. ✅ Sinh PDF — **LIVE**: `GET /documents/generated/{id}/pdf` qua Gotenberg (LibreOffice headless); nút "Tải PDF". Đã chạy `gotenberg` + `DOCUMENT_PDF_ENABLED=true`.
2. 🚫 Ký số. 3. ✅ Gửi email — **LIVE**: `POST /documents/generated/{id}/email` gửi LINK chia sẻ bảo mật qua Gmail SMTP sẵn có; nút "Gửi email". Đã bật `DOCUMENT_MAIL_ENABLED=true`. 4. ✅ Chia sẻ bảo mật cho KH — link token băm (SHA-256), hạn dùng + giới hạn lượt tải + thu hồi; endpoint công khai `GET /documents/shared/{token}` (permitAll); nút "Tạo link chia sẻ" ở màn tạo tài liệu.
