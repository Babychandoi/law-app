...
> vẫn nên chạy **UAT chính thức** trước khi ký nghiệm thu cuối. Điểm ước ~**8,5/10**.
>
> 📄 **Bản nghiệm thu sạch để gửi khách hàng:** `Bao-cao-nghiem-thu-Admin-Luat-POIP.md`. Các mục
> §1/§3/§5/§12 bên dưới là **đánh giá gốc 26/07 (baseline 4,8/10), KHÔNG còn phản ánh hiện trạng** —
> giữ để đối chiếu; trạng thái hiện tại xem §0 + §9.
...
- **axe-core (`e2e/a11y.spec.ts`)**: 0 lỗi `color-contrast` trên 5 màn (đăng nhập/Khách hàng/Người đăng ký/CRM/Tin tức) → đóng P0.8.
...
- Accessibility: **đã có cổng chặn axe trên mỗi PR** (job `a11y`, quét trang đăng nhập + preview offline) và bộ E2E quét 5 màn thật; **chưa** mở rộng axe ra toàn bộ màn quản trị.
...
- Các mục P2 chưa làm: audit log, lịch sử thay đổi, command palette, autosave, draft/version, preview đa thiết bị, visual regression, analytics.
...
- ✅ **E2E vào CI**: `.github/workflows/e2e.yml` (5-màn thật, chạy thủ công/khi có secrets) **+ cổng chặn a11y tự động mỗi PR** (job `a11y` trong `ci.yml`, không cần secret).
- ✅ **Backend có test JUnit đầu tiên** (ApiMeta) — CI `mvn package` chạy test.

### Còn tồn (trung thực) — thuộc P2/hạ tầng test
- ⬜ Test coverage breadth: gateway/chat/crm/document chưa có test; chưa có test mobile/keyboard.
- ⬜ E2E 5-màn thật vẫn chạy thủ công/khi có secrets (cron tự động cho tới khi set secrets E2E_*); **cổng chặn axe public đã tự động trên mỗi PR**.

### Số liệu test (chính xác, cập nhật)
- **70** unit test frontend (15 suite) — gồm test DataTable (ẩn cột/mật độ + saved-filter round-trip) và **7 test `contrast` (WCAG)**.
- Playwright: bộ E2E 5-màn thật (cần secrets) + **cổng chặn a11y public** (project `public`, chạy trong CI không cần secret).
...
1. ✅ Saved view / saved filter. *(view lưu tìm kiếm + sort + trang + bộ lọc (ảnh chụp query URL) cùng mật độ + cột ẩn; unit test round-trip. CRM dùng state nội bộ nên chỉ lưu mật độ/cột.)*
2. ✅ Column visibility. *(ẩn/hiện cột; thẻ mobile mặc định tôn trọng cột ẩn.)*
3. ✅ Table density. *(nút Thoáng/Gọn, lưu localStorage.)*
4. ✅ Audit log. *(`audit_logs` + `AuditService`, hook 8 thao tác nhạy cảm; `GET /audit-logs` admin-only + trang Nhật ký kiểm toán; kiểm chứng prod.)*
5. ✅ Lịch sử thay đổi dữ liệu. *(change-history: diff `{"field":{"old":..,"new":..}}` cho đổi vai trò/trạng thái.)*
6. ✅ Global search / command palette. *(⌘/Ctrl+K + nút Navbar; a11y đầy đủ; 3 unit test.)*
7. ✅ Autosave cho Service Editor. *(nháp localStorage debounce + banner khôi phục; xóa nháp sau khi lưu.)*
8. ✅ Draft và version history. *(bài viết: `news_versions` + xem/khôi phục; Dịch vụ là follow-up.)*
9. ✅ Preview theo desktop/tablet/mobile. *(nút desktop/tablet/mobile giới hạn bề rộng — mô phỏng bề rộng, chưa emulate breakpoint đầy đủ.)*
10. ⚠️ Visual regression test. *(`e2e/visual.spec.ts` + job `visual` trong image Playwright + workflow tự sinh baseline. **Cần merge baseline vào `main`** để gate chạy trên `main`.)*
11. ✅ Accessibility test trong CI. *(cổng axe public mỗi PR (`a11y-public.spec.ts`); `textOn` tách `shared/utils/contrast.ts` + 7 unit test; đã bắt lỗi thật #8B5CF6 4,19:1 → `#000000` 4,96:1.)*
12. ✅ Theo dõi analytics cho luồng admin quan trọng. *(nội bộ, tự lưu DB — `/audit-logs/stats` + trang Thống kê hoạt động (KPI + biểu đồ theo ngày + theo loại). Kiểm chứng prod 200.)*
